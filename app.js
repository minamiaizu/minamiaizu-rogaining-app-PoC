  ...
  if (!orientationManager) {
    ...
    orientationManager.onUpdate = (data) => {
      ...
      if (typeof debugOverlay !== "undefined" && debugOverlay) {
        debugOverlay.sensorData = {
          alpha: data.rawHeading || 0,
          beta: data.beta || 0,
          gamma: 0,
          webkitCompassHeading: data.heading,
...
function switchView(view){
  ...
  if (view==='compass'){ 
    ...
    // デバッグボタンを追加（まだ存在しない場合）
    const compassView = document.getElementById("compass-view");
    if (compassView && !compassView.querySelector(".debug-buttons")) {
      addDebugButtons("compass-view", "compass");
    }
  }
  ...
  if (view==='sonar'){
    ...
    debugLog('ソナービュー開始');
    // デバッグボタンを追加（まだ存在しない場合）
    const sonarView = document.getElementById("sonar-view");
    if (sonarView && !sonarView.querySelector(".debug-buttons")) {
      addDebugButtons("sonar-view", "sonar");
    }
  } else {
...
(function init(){
  ...
  // デバッグボタンの初期化（デバッグシステムが読み込まれた後に実行）
  setTimeout(() => {
    if (typeof addDebugButtons === 'function') {
      // 初期ビューがコンパスの場合
      if (currentView === 'compass') addDebugButtons('compass-view', 'compass');
      // 初期ビューがソナーの場合
      if (currentView === 'sonar') addDebugButtons('sonar-view', 'sonar');
      // 初期ビューがARの場合
      if (currentView === 'ar') addDebugButtons('ar-view', 'AR');
      debugLog('✅ デバッグボタン初期化完了');
    }
  }, 500);
})();
/* ======== Debug System (共通デバッグシステム) ======== */
 ...
 * デバッグオーバーレイを作成・表示
 */
function createDebugOverlay(viewName) {
  if (!debugOverlay.element) {
    ...
    debugOverlay.element = overlay;
  }
  
  debugOverlay.currentView = viewName;
  debugOverlay.element.style.display = 'block';
  debugOverlay.isVisible = true;
  
  // デバッグ情報の更新を開始
  startDebugUpdate();
  
  debugLog(`デバッグオーバーレイ表示: ${viewName}`);
}
...
function hideDebugOverlay() {
  if (debugOverlay.element) {
    debugOverlay.element.style.display = 'none';
  }
  debugOverlay.isVisible = false;
  stopDebugUpdate();
  debugLog('デバッグオーバーレイ非表示');
}

/**
 * デバッグオーバーレイの表示切替
 */
function toggleDebugOverlay(viewName) {
  if (debugOverlay.isVisible && debugOverlay.currentView === viewName) {
    hideDebugOverlay();
  } else {
    createDebugOverlay(viewName);
  }
}
...
function collectDebugData() {
  const data = {
    timestamp: new Date().toISOString(),
    view: debugOverlay.currentView || currentView,
    ...
    position: currentPosition ? {
      ...
      elevation: (currentPosition.elevation || 0).toFixed(1) + 'm'
    } : null,
    
    // OrientationManager情報
    orientation: orientationManager ? orientationManager.getDebugInfo() : {
      heading: Math.round(smoothedHeading),
      confidence: 'N/A',
      accuracy: 'N/A',
      mode: 'fallback',
      platform: IS_IOS ? 'iOS' : 'Other',
      updates: 0,
      beta: Math.round(devicePitch)
    },
    
    // センサー生データ
    sensors: {
      alpha: debugOverlay.sensorData.alpha?.toFixed(1) + '°' || 'N/A',
      beta: debugOverlay.sensorData.beta?.toFixed(1) + '°' || 'N/A',
      gamma: debugOverlay.sensorData.gamma?.toFixed(1) + '°' || 'N/A',
      webkitCompassHeading: debugOverlay.sensorData.webkitCompassHeading?.toFixed(1) + '°' || 'N/A',
      webkitCompassAccuracy: debugOverlay.sensorData.webkitCompassAccuracy?.toFixed(1) + '°' || 'N/A'
    },
  ...
  if (currentPosition) {
    ...
    data.bearings = bearingData.slice(0, 3).map(item => ({
      ...
      // 相対方位（現在の向きからの角度差）
      relative: Math.round(((item.brng - smoothedHeading + 540) % 360) - 180) + '°'
    }));
  }
...
function startDebugUpdate() {
  stopDebugUpdate();
  debugOverlay.updateInterval = setInterval(() => {
    if (debugOverlay.isVisible && debugOverlay.element) {
      const data = collectDebugData();
      const html = formatDebugData(data);
...
function stopDebugUpdate() {
  if (debugOverlay.updateInterval) {
    clearInterval(debugOverlay.updateInterval);
    debugOverlay.updateInterval = null;
  }
}
  ...
  if (orientationManager) {
    orientationManager.init().then(success => {
      if (success) {
        ...
        debugLog('⚠️ センサーリロード中に問題が発生');
        alert('センサーのリロード中に問題が発生しました');
      }
    });
  } else {
    // OrientationManagerが未初期化の場合は初期化
    debugLog('OrientationManagerを初期化中...');
    startOrientation();
  }
}
 ...
 * デバッグボタンを特定のコンテナに追加
 */
function addDebugButtons(containerId, viewName) {
  const container = document.getElementById(containerId);
  if (!container) {
    debugLog(`⚠️ コンテナ ${containerId} が見つかりません`);
    return;
  }
  ...
  debugBtn.onclick = () => toggleDebugOverlay(viewName);
  ...
  reloadBtn.onclick = () => reloadSensors();
  ...
  debugLog(`✅ デバッグボタンを ${viewName} に追加`);
}
